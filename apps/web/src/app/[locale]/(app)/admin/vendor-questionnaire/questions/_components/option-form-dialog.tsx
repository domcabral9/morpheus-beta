...
import type { VendorQuestionOptionAdmin } from "@/lib/vendor-questionnaire-admin-types";
import {
  Dialog,
  DialogContent,
  DialogFooter,
...
type OptionFormDialogProps = {
  ...
  onSaved: (option: VendorQuestionOptionAdmin) => void;
} & ({ mode: "create"; option?: undefined } | { mode: "edit"; option: VendorQuestionOptionAdmin });
...
export function OptionFormDialog({
  ...
  async function onSubmit(values: OptionFormOutput) {
    try {
      const saved = option
        ? await api.patch<VendorQuestionOptionAdmin>(`/vendors/admin/options/${option.id}`, values)
        : await api.post<VendorQuestionOptionAdmin>(
            `/vendors/admin/questions/${questionId}/options`,
            values,
          );
