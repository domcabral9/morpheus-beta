...
import {
  VENDOR_CHOICE_TYPES,
  VENDOR_QUESTION_TYPES,
  type VendorQuestionAdmin,
  type VendorQuestionCategoryAdmin,
  type VendorQuestionOptionAdmin,
} from "@/lib/vendor-questionnaire-admin-types";
...
const basicSchema = z.object({
  ...
  type: z.enum(VENDOR_QUESTION_TYPES),
...
function BasicInfoForm({
  ...
  question: VendorQuestionAdmin;
  categories: VendorQuestionCategoryAdmin[];
  onSaved: (question: VendorQuestionAdmin) => void;
}) {
  ...
  async function onSubmit(values: BasicFormOutput) {
    try {
      const updated = await api.patch<VendorQuestionAdmin>(`/vendors/admin/questions/${question.id}`, {
        ...values,
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select value={field.value} onValueChange={field.onChange}>
                ...
                <SelectContent>
                  {VENDOR_QUESTION_TYPES.map((value) => (
...
function OptionsSection({
  ...
  question: VendorQuestionAdmin;
  onChanged: (question: VendorQuestionAdmin) => void;
}) {
  const t = useTranslations("AdminVendorQuestionnaire");
  const api = useApi();

  const [dialogState, setDialogState] = React.useState<
    { mode: "create" } | { mode: "edit"; option: VendorQuestionOptionAdmin } | null
  >(null);
  const [deleteTarget, setDeleteTarget] = React.useState<VendorQuestionOptionAdmin | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`/vendors/admin/options/${deleteTarget.id}`);
...
function QuestionDetailContent() {
  ...
  const [question, setQuestion] = React.useState<VendorQuestionAdmin | null | undefined>(undefined);
  const [categories, setCategories] = React.useState<VendorQuestionCategoryAdmin[]>([]);
  const [error, setError] = React.useState<string | null>(null);

  React.useEffect(() => {
    Promise.all([
      api.get<VendorQuestionAdmin[]>("/vendors/admin/questions"),
      api.get<VendorQuestionCategoryAdmin[]>("/vendors/admin/categories"),
    ])
      .then(([questions, cats]) => {
    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      {question && (
        <>
          ...
          {VENDOR_CHOICE_TYPES.has(question.type) && (
...
export default function AdminVendorQuestionDetailPage() {
  return (
