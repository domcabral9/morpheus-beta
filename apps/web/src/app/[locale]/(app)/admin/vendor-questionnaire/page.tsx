...
function CategorySection({
  ...
  category: VendorQuestionCategoryAdmin;
  questions: VendorQuestionAdmin[];
  onEdit: () => void;
        ...
        {open && (
          <div className="flex flex-col gap-1 border-t pt-3">
            ...
            {questions.map((question) => (
              <Link
                key={question.id}
                href={`/admin/vendor-questionnaire/questions/${question.id}`}
              ...
              >
                ...
                <Badge variant="secondary" className="shrink-0 text-xs">
                  {t(`question.types.${question.type}`)}
                </Badge>
              </Link>
            ))}
          </div>
...
function VendorQuestionnaireAdminContent() {
  const t = useTranslations("AdminVendorQuestionnaire");
  const api = useApi();

  const [categories, setCategories] = React.useState<VendorQuestionCategoryAdmin[] | null>(null);
  const [questions, setQuestions] = React.useState<VendorQuestionAdmin[] | null>(null);
  const [error, setError] = React.useState<string | null>(null);
  const [createCategoryOpen, setCreateCategoryOpen] = React.useState(false);
  const [editingCategory, setEditingCategory] = React.useState<VendorQuestionCategoryAdmin | null>(
    null,
  );
...
export default function AdminVendorQuestionnairePage() {
  return (
    <AdminSectionGate permission="vendors:manage">
      <VendorQuestionnaireAdminContent />
    </AdminSectionGate>
  );
}
