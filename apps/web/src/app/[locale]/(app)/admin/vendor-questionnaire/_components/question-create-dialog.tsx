...
interface QuestionCreateDialogProps {
  ...
  onCreated: (question: VendorQuestionAdmin) => void;
}
...
export function QuestionCreateDialog({
  ...
  const needsOptions = VENDOR_CHOICE_TYPES.has(type);
  ...
  async function onSubmit(values: QuestionCreateFormOutput) {
    ...
    try {
      const created = await api.post<VendorQuestionAdmin>("/vendors/admin/questions", {
        categoryId,
        text: values.text,
