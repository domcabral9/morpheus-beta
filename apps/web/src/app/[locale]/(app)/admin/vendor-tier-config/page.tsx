...
import type { VendorTierConfigSummary } from "@/lib/vendor-tier-config-types";
import { AdminSectionGate } from "../_components/section-gate";
import { ConfigCreateDialog } from "./_components/config-create-dialog";

function VendorTierConfigAdminContent() {
  const t = useTranslations("AdminVendorTierConfig");
  const api = useApi();

  const [configs, setConfigs] = React.useState<VendorTierConfigSummary[] | null>(null);
  const [error, setError] = React.useState<string | null>(null);
  const [createOpen, setCreateOpen] = React.useState(false);

  const load = React.useCallback(() => {
    api
      .get<VendorTierConfigSummary[]>("/vendors/admin/tier-configs")
      .then((result) => {
        setConfigs(result);
        setError(null);
      })
                ...
                {configs.map((config) => (
                  <TableRow key={config.id}>
                    <TableCell className="font-medium">
                      <Link href={`/admin/vendor-tier-config/${config.id}`} className="hover:underline">
                        {config.name}
                      </Link>
                    </TableCell>
...
export default function AdminVendorTierConfigPage() {
  return (
    <AdminSectionGate permission="vendors:manage">
      <VendorTierConfigAdminContent />
    </AdminSectionGate>
  );
}
