...
> **Status:** Etapa 3 — Autenticação e RBAC. Login local (email/senha) e SSO via SAML genérico
> (desabilitado por padrão), JWT de acesso curto + refresh token rotativo e revogável, RBAC
> aplicado globalmente por padrão (`@Public()` para isentar rotas), e a primitiva de Separação de
> Funções. Ainda não há CRUD de avaliações/questionários (Etapa 4).
