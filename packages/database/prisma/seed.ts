...
// --- Fluxo de aprovação padrão (dados compartilhados por todos os tenants) ---
const stepDefs = [
...
interface SeedFullTenantParams {
  slug: string;
  name: string;
  logoUrl?: string;
  adminEmail: string;
  ...
  controlByKey: Map<string, { id: string }>;
  seedPasswordHash: string;
}

async function seedFullTenant(params: SeedFullTenantParams) {
  const {
    slug,
    name,
    logoUrl,
  ...
  for (const [index, categoryName] of QUESTION_CATEGORIES.entries()) {
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2003") {
      console.warn(
        `[seed] Pulando reset de perguntas de "${slug}": já existem Assessments respondidas referenciando-as.`,
      );
    } else {
      throw error;
    }
  ...
  // --- Vínculo de perguntas aos controles da biblioteca -------------------------
  ...
  return { tenant, adminUser, regularUser };
}
...
async function main() {
  ...
  const demoResult = await seedFullTenant({
    slug: "demo",
    name: "Empresa Demo",
    adminEmail: "admin@morpheus.demo",
    adminName: "Administrador Demo",
    userEmail: "usuario@morpheus.demo",
  ...
  // arquivos estáticos já preparados em apps/web/public/tenant-logos/.
  const EXTRA_TENANTS = [
    { slug: "zion", name: "Zion", logoUrl: "/tenant-logos/zion.png" },
    { slug: "matrix", name: "Matrix", logoUrl: "/tenant-logos/matrix.png" },
    { slug: "machine-city", name: "Cidade das Máquinas", logoUrl: "/tenant-logos/cidade-das-maquinas.png" },
  ];

  const extraResults = [];
  for (const extra of EXTRA_TENANTS) {
    extraResults.push(
      await seedFullTenant({
        slug: extra.slug,
        name: extra.name,
        logoUrl: extra.logoUrl,
        adminEmail: `admin@${extra.slug}.morpheus.demo`,
        adminName: `Administrador ${extra.name}`,
        userEmail: `usuario@${extra.slug}.morpheus.demo`,
        userName: `Usuário ${extra.name}`,
  ...
  console.log("Seed concluído:", {
    tenants: [demoResult.tenant.slug, ...extraResults.map((r) => r.tenant.slug)],
    ...
    logins: [
      demoResult.adminUser.email,
      demoResult.regularUser.email,
      ...extraResults.flatMap((r) => [r.adminUser.email, r.regularUser.email]),
    ],
  });
}
